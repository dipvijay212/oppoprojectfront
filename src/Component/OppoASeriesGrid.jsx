...
const OppoASeriesGrid = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        // Filter using name since 'series' field doesn't exist
        const findNProducts = res.data.filter(
          (product) =>
            product.category === "Smartphone" &&
            product.name.toLowerCase().includes("oppo a")
        );
        setProducts(findNProducts);
      })
      .catch((err) => {
        console.error("Error fetching Find N Series products:", err);
      });
  }, []);
    ...
    <div className="findn-grid-container">
      {products.map((phone, index) => (
         <Link
            ...
            >
        <div className="phone-box" key={phone._id || index}>
          ...
          <div className="info">
            ...
            <a href={`/product/${phone._id}`} className="learn-more">
              Learn more
            </a>
          </div>
        </div>
...
export default OppoASeriesGrid;
